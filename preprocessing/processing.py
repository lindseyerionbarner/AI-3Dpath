...
from skimage.draw import polygon_perimeter as ski_polygon_perimeter
from skimage.measure import label as ski_label

from .datamodel import Slide
from .util import ProgressBar, LogStyleAdapter
...
def rgb2gray(rgb: np.ndarray) -> np.ndarray:
    ...
    #     / 1.5)
    ## Custom defined "gray" by LB below: 10-27-21 for images with ch0 in R, ch1 in G, B empty
    gray = 1.0 * rgb[::, ::, 0] + rgb[::, ::, 1]
    gray[gray < 0] = 0
    gray[gray > 255] = 255
    return gray
...
def create_tumor_outline(slide: Slide, level, bounds=None):
    """Create outline of a  tumor mask for a slide or slide section.
    ...
    for i, annotation in enumerate(slide.annotations):
        ...
        rr, cc = ski_polygon_perimeter(r, c, shape=mask.shape)
        mask[rr, cc] = 1
