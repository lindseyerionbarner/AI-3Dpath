...
class SlideManager:
    ...
    def __init__(self, *, cam16_dir=None, cam17_dir=None):
        ...
        if cam16_dir is None and cam17_dir is None:
        ...
        if cam16_dir is not None:
            ...
            self._path['cam16'] = {
                ...
                'annotations': os.path.join(cam16_dir, 'training/annotations'),
                'test': os.path.join(cam16_dir, 'test/images'), # NEW
                'test_annotations': os.path.join(cam16_dir, 'test/annotations'), # NEW
                'otsu': os.path.join(cam16_dir, 'training/otsu_thresholds.csv')
            }
            self.__load_cam16()

        if cam17_dir is not None:
            cam17_dir = os.path.expanduser(cam17_dir)
            self._path['cam17'] = {
                'dir': cam17_dir,
                'images': os.path.join(cam17_dir, 'training'),
                'annotations': os.path.join(cam17_dir, 'training/_annotations'),
                'stages': os.path.join(cam17_dir, 'training/stage_labels.csv'),
                'otsu': os.path.join(cam17_dir, 'training/otsu_thresholds.csv')
            }
            self.__load_cam17()
    ...
    def __load_cam16(self):
        ...
        # negative slides
        otsu_thresholds = defaultdict(dict)
        try:
            with open(self._path['cam16']['otsu'], 'r') as f:
                reader = csv.DictReader(f)
                for line in reader:
                    otsu_thresholds[line['name']][int(line['level'])] = float(
                        line['threshold'])
        ...
        slide_files = find_files('*.tif', self._path['cam16']['positive'])
        for file_name, slide_path in sorted(slide_files.items()):
            slide_name, _, _ = file_name.partition('.')
        ...
        # test slides # NEW
        slide_files = find_files('*.tif', self._path['cam16']['test'])
        for file_name, slide_path in sorted(slide_files.items()):
            slide_name, _, _ = file_name.partition('.')
    ...
    def __load_cam17(self):
        """Load CAMELYON17 slides."""
        logger.info('Loading CAMELYON17 slides')
        # Read pN-stages from csv file
        with open(self._path['cam17']['stages'], 'r', ) as f:
            reader = csv.DictReader(f)
            stages = {line['patient']: line['stage'] for line in reader}

        otsu_thresholds = defaultdict(dict)
        try:
            with open(self._path['cam17']['otsu'], 'r') as f:
                reader = csv.DictReader(f)
                for line in reader:
                    otsu_thresholds[line['name']][int(line['level'])] = float(
                        line['threshold'])
        ...
        slide_files = find_files('*.tif', self._path['cam17']['images'])
        for file_name, slide_path in sorted(slide_files.items()):
            slide_name, _, _ = file_name.partition('.')
            annotation_path = os.path.join(self._path['cam17']['annotations'],
                                           f'{slide_name}.xml')
            if not os.path.exists(annotation_path):
            ...
            if not slide.has_tumor:
                self.negative_slides += (slide,)
            elif slide.is_annotated:
                self.annotated_slides += (slide,)
